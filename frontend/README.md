...
Los hitos de Historia se editan en `src/data/historyTimeline.json`: `year`, `title`, `description`, `image` e `imageAlt`. Las fotos se guardan en `public/images/`; `image` contiene la ruta relativa, por ejemplo `images/asociacion.jpg`. Cada hito alterna a derecha e izquierda en escritorio; en móvil, las tarjetas se muestran en una columna junto al tronco. La vista recorre los hitos desde el más reciente en la copa hasta el origen en las raíces. El JSON se mantiene ordenado del más antiguo al más reciente; el componente invierte una copia para mostrarlo. Las fechas actuales son ilustrativas y están señaladas como tales junto a la introducción y en el aviso final. Las fotos actuales son referencias del archivo, no documentación verificada de esos años. Mantener `provisional: true` hasta verificar todas las fechas y fotografías; después puede cambiarse a `false`. El diseño está en `src/components/HistoryTimeline.jsx` y `HistoryTimeline.css`. El relato institucional original sigue disponible bajo «Leer el relato completo de nuestra historia».

El árbol de `src/components/HistoryTree.jsx` combina cuatro ilustraciones WebP con transparencia: copa frondosa, tronco con corteza, marco de ramas y raíces amplias. Se guardan en `public/images/history-tree/` y se declaran en `src/data/historyTreeAssets.json`. Las ramas rodean cada tarjeta y se reflejan según el lado; el tronco se adapta a la altura del contenido. La fotografía acompaña al texto en escritorio y se sitúa debajo en celulares estrechos. El enlace «Comenzar en la semilla» lleva al origen y funciona con teclado. El mapa `treeTitles` en `HistoryTimeline.jsx` contiene los títulos botánicos; los eventos nuevos usan su título del JSON como alternativa.

Las ilustraciones son decorativas y no se anuncian a lectores de pantalla. Los textos y las fotos permanecen editables. No se incorporan dependencias ni servicios externos durante la visita; `verify:build` comprueba las cuatro rutas, dimensiones y transparencia WebP. La procedencia y los prompts seleccionados se documentan en [Ilustraciones del árbol](docs/history-tree-artwork.md).
