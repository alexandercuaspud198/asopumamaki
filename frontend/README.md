...
El árbol de `src/components/HistoryTree.jsx` acompaña la lectura: empieza con semillas y desarrolla un brote, raíces, ramas, hojas y frutos al avanzar por los hitos. Sus seis etapas se distribuyen entre los eventos del JSON. En escritorio, los botones bajo el árbol permiten ir a cada fecha con ratón o teclado; en móvil, un panel compacto mantiene visible la etapa durante el desplazamiento. Las animaciones y el desplazamiento suave respetan la preferencia del dispositivo de reducir el movimiento. El árbol es un SVG local y no requiere dependencias ni imágenes adicionales.
