...
El árbol de `src/components/HistoryTree.jsx` es una ilustración continua de SVG: copa con frutos, tronco, ramas por hito y raíces con una semilla. El tronco se adapta a la altura del contenido; las hojas, la copa y las raíces mantienen sus proporciones. Las variables CSS `--branch-width` y `--root-width` regulan las ramas y la base: alcanzan 640 y 900 píxeles en escritorio. Las ramas se extienden hacia el lado libre de cada tarjeta; la primera es compacta para respetar el encabezado. En móvil, ambas dimensiones se ajustan al ancho disponible. La fotografía acompaña al texto en escritorio y se sitúa debajo en celulares estrechos. El enlace «Comenzar en la semilla» lleva al origen y funciona con teclado. El mapa `treeTitles` en `HistoryTimeline.jsx` contiene los títulos botánicos; los eventos nuevos usan su título del JSON como alternativa. La ilustración permanece completa, no necesita animación, imágenes adicionales ni dependencias externas.
